...
const FeatureSidebar = ({ setProducts }: FeatureSidebarProps) => {
  ...
  const filterdProduct = allProducts.filter((product: any) => product.page === 'shop_1');
  ...
  const categoryFilter = filterdProduct.map((product: any) => product.category);
  const amenitiesFilter = filterdProduct.map((product: any) => product.amenities);
  const languageFilter = filterdProduct.map((product: any) => product.language);

  const allCategory = ['All Package Types', ...new Set(categoryFilter)];
  const allAmenities = ['All Activities', ...new Set(amenitiesFilter)];
  const allLanguage = ['All Destinations', ...new Set(languageFilter)];
  ...
  const filterProducts = ({ category, amenities, language, rating }: FilterCriteria) => {
    ...
    if (category && category !== 'All Package Types') {
      filteredProducts = filteredProducts.filter((product: any) => product.category === category);
    }

    if (amenities && amenities !== 'All Activities') {
      filteredProducts = filteredProducts.filter((product: any) => product.amenities === amenities);
    }

    if (language && language !== 'All Destinations') {
      filteredProducts = filteredProducts.filter((product: any) => product.language === language);
    }

    if (rating !== null) {
      filteredProducts = filteredProducts.filter((product: any) => product.review === rating);
    }
  ...
  // handle Price
  const maxPrice = travel_package_data.reduce((max, item) => {
  ...
  useEffect(() => {
    const filterPrice = travel_package_data.filter((j) => j.price >= priceValue[0] && j.price <= priceValue[1]);
    ...
    <div className="col-xl-3 col-lg-4 order-last order-lg-first">
      <div className="tg-filter-sidebar mb-40 top-sticky">
        <div className="tg-filter-item">
          ...
          {/* category */}
          <h4 className="tg-filter-title mb-15">Package Type</h4>
          <div className="tg-filter-list">
            <ul>
              {allCategory.map((category, i) => (
                <li key={i} onClick={() => handleCategory(category as string)}>
                  <div className="checkbox d-flex">
                    <input className="tg-checkbox" type="checkbox" checked={category === categorySelected} readOnly id={`cat_${i}`} />
                    <label htmlFor={`cat_${i}`} onClick={() => handleCategory(category as string)} className="tg-label">{category as string}</label>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div className="tg-filter-list">
            <ul>
              {allAmenities.map((amenities, i) => (
                <li key={i} onClick={() => handleAmenities(amenities as string)}>
                  <div className="checkbox d-flex">
                    <input className="tg-checkbox" type="checkbox" checked={amenities === amenitiesSelected} readOnly id={`amenities_${i}`} />
                    <label className="tg-label" htmlFor={`amenities_${i}`} onClick={() => handleAmenities(amenities as string)}>{amenities as string}</label>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div className="tg-filter-list">
            <ul>
              {allLanguage.map((language, i) => (
                <li key={i} onClick={() => handleLanguage(language as string)}>
                  <div className="checkbox d-flex">
                    <input className="tg-checkbox" type="checkbox" checked={language === languageSelected} readOnly id={`language_${i}`} />
                    <label className="tg-label" htmlFor={`language_${i}`} onClick={() => handleLanguage(language as string)}>{language as string}</label>
                  </div>
                </li>
              ))}
            </ul>
